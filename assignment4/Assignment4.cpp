...
#define DISABLE_OPENGL_SUBROUTINES
	...
	lightProperties->singleColor = glm::vec4(0.f, 0.f, 1.f, 1.f);
	pointLight = std::make_shared<HemisphereLight>(std::move(lightProperties), glm::vec4(0.f,1.f,0.f,1.f));
...
#define DISABLE_OPENGL_SUBROUTINES
	...
	pointLight = std::make_shared<DirectionalLight>(std::move(lightProperties), glm::vec3(-1, -.5, 0));
	pointLight->SetPosition(glm::vec3(20.f, 13.f, 20.f));
	scene->AddLight(pointLight);
